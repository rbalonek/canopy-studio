...
export function ChannelPreview({
  brand = 'Acme Dental',
  handle = 'acmedental',
  headline = 'Saturday dental care for busy families',
  body = 'Same-day crowns. Insurance welcome. Book a weekend slot with Dr. Patel — our calendar opens this Friday.',
}: Props) {
  const [ch, setCh] = useState<Channel>('ig');
  ...
  const avatar = (
    <div
      style={{
        ...
        height: 32,
        borderRadius: 999,
        background: 'linear-gradient(135deg,var(--accent),#0E8A80)',
        display: 'flex',
        alignItems: 'center',
  ...
  const creative = (
    <div style={{ width: '100%', aspectRatio: '1 / 1' }}>
      <AdThumb seed={12} brand={brand} kind="photo" headline={headline} h="100%" w="100%" />
    </div>
  );
  ...
  return (
    <div className="card card-pad stack gap-10">
      ...
      {ch === 'ig' && (
        ...
        >
          ...
          <div style={{ padding: '0 12px 10px', fontSize: 12, lineHeight: 1.4 }}>
            ...
            <div>
              <b>{handle}</b> {body}{' '}
              <span style={{ color: '#a8a8a8' }}>#family #weekend #dentist</span>
            </div>
        ...
        >
          <div style={{ padding: '10px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: '#B0B3B8' }}>
                Sponsored · <span style={{ color: '#B0B3B8' }}>🌐</span>
              </div>
            </div>
            <span style={{ fontSize: 16 }}>⋯</span>
          </div>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontSize: 11, color: '#B0B3B8', textTransform: 'uppercase' }}>
                {handle}.com
              </span>
          ...
          >
            {['👍 Like', '💬 Comment', '↗ Share'].map((x) => (
              <span key={x} style={{ padding: '6px 0' }}>
                {x}
              </span>
            ))}
          </div>
        ...
        >
          <div
            style={{
              width: '100%',
              aspectRatio: '9 / 16',
              background: 'linear-gradient(160deg,#1b1b24 0%,#2a5f8d 100%)',
              position: 'relative',
              color: '#fff',
              overflow: 'hidden',
            }}
          >
            <div
              style={{
                ...
                inset: 0,
                background:
                  'repeating-linear-gradient(135deg,transparent 0 20px,rgba(255,255,255,0.05) 20px 21px)',
              }}
            />
            <div
              style={{
      ...
      <div className="meta" style={{ textAlign: 'center', fontSize: 11 }}>
        Channel-accurate preview · reflects selected variant
      </div>
    </div>
  );
}
